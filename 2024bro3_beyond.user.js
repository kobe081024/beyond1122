...
function addGlobalStyles() {
	...
	var css = "\
		...
		.menu > li { \
			...
			width: 105px; \
			height: 7px; \
			background-image: url('/20240502-01/extend_project/w945/img/menu_mark.jpg'); \
			z-index: 9902; \
		} \
		.menu > li a { \
			display: block; \
						...
						q$("div[class='front'] span[class='status_frontback']").append(
							"<span class='status_levelup'>" +
								"<a href='" + BASE_URL + "/card/status_info.php?cid=" + cid + "'>" +
									"<img src='/20240502-01/pqAx4lHWriQT7gRLz6jk82UCPQLJKZ0bFg48Li5CNA0=/20240502-01/extend_project/w945/img/card/common/btn_levelup.png' alt='ステータス強化' title='ステータス強化' class='levelup'>" +
								"</a>" +
							"</span>"
						);
					}
		...
		[
			...
			[
				['城トップ', facurl + '/castle.php'],
                 ['部隊戦', BASE_URL +  '/npc_expedition/?stage_category=2',
					[
						 ['EX1部隊戦5', BASE_URL + '/npc_expedition/index.php?stage_category=2&stage_id=2005'],
				　　　　	 ['EX2部隊戦9', BASE_URL + '/npc_expedition/index.php?stage_category=2&stage_id=2009'],
                         ['EX3部隊戦12', BASE_URL + '/npc_expedition/index.php?stage_category=2&stage_id=2012'],
				],
                 ],
                  ['武将戦', BASE_URL +  '/npc_expedition/?stage_category=1',
					[
						 ['EX1武将戦7', BASE_URL + '/npc_expedition/index.php?stage_category=1&stage_id=1007'],
				　　　　	 ['EX2武将戦15', BASE_URL + '/npc_expedition/index.php?stage_category=1&stage_id=1015'],
                         ['EX3武将戦20', BASE_URL + '/npc_expedition/index.php?stage_category=1&stage_id=1020'],
				],
                 ],
				['兵士管理', facsturl,
					[
				...
				for (var i = 0; i < cards.length; i++) {
					...
					q$("div[class^='left']", cards[i]).eq(0).append(
						'<a href=' + BASE_URL + '/union/lvup.php?cid=' + card_id + '>' +
							'<img style="width: 100%; cursor: pointer;" src="/pqAx4lHWriQT7gRLz6jk82UCPQLJKZ0bFg48Li5CNA0=/20240502-01/extend_project/w945/img/union/btn_levelupskill_mini.png" alt="ベースカードをこのカードに変更" title="ベースカードをこのカードに変更">' +
						'</a>'
					);
				}
			}
			...
			<div id='beyond_tabs' style='margin: 4px; font-size:10px;'> \
				...
				<div id='tab-deck'> \
					...
					<div style='margin-left: 8px;'> \
						...
						<div><input type='checkbox' id='" + DECK_18 + "'><label for='" + DECK_18 + "'>デッキ：警護一括デッキセット機能を追加</label></input></div> \
				...
				"<div id='search-result-div' class='roundbox' style='display: none;'>" +
					"<div style='padding: 2px;'>" +
						...
						"<div style='overflow-y: auto; max-height: 1050px;'>" +
							"<table id='search-result' style='font-size: 1pt; display: none; margin-right: 50px;'>" +
							"</table>" +
						"</div>" +
					"</div>" +
				"</div>" +
				...
				.done(function(res) {
					...
					if (count > max) {
						...
						for (var i = 0; i < result.length; i++) {
							...
							var tr = "<tr>" +
								"<td class='tpad'>" + result[i].name + "</td>" +
								"<td class='tpad'>" + result[i].cardid + "</td>" +
...
/*
                    "<select id='deck_mode' style='margin: 4px;'>" +
                        "<option value='usual'>通常デッキ</option>" +
                        "<option value='patrol'>警護デッキ</option>" +
                    "</select>" +
*/
                    "<input id='multi_card_set' type='button' style='font-size: 20px;' value='実行'></input>" +
...
console.log("card_no: " + card_no + " already used!");
                                    continue;
                                }

                                used.push(card_no);
console.log("puehed used: " + used);
console.log("befor choosed: " + choosed[general_ct]);
...
//console.log("dummy array: " + {cardno:"1"});
console.log("after pushed generals: " + generals[general_ct]);
                                general_ct ++;
							}
						}
...
//							var used = [];
//							var choosed = [];
console.log("candidate sorted!");
                            for (i = 0; i < generals.length; i++) {
								if (freecost < 1 || freecost < generals[i].cost) {
console.log("No more general can up! i: " + i);
                                    break;
								}
/*
...
console.log("choosed length: " + choosed.length);
                            if (choosed.length > 0) {
...
console.log("POST: cardno: " + targets[count - 1].cardno);
//					var target_url = BASE_URL + '/card/deck.php';
...
　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　var skill_info = getSkillInfo(use_skill, q$('div.set a.control__button--deck-set-small', elembase).attr('href'));
			...
			if (g_history_mode == false && is_easy_deck_set == true) {
				...
				if (is_active == true) {
					el.before(
						"<div>" +
							"<input type='button' id='deck_domestic_" + i + "' style='font-size: 8px;' value='内政'></input>" +
							"<input type='button' id='deck_set_" + i + "' style='font-size: 12px;' value='配置'></input>" +
					...
					el.before(
						"<div>" +
							"<input type='button' style='font-size: 8px;' value='内政' disabled></input>" +
							"<input type='button' style='font-size: 12px;' value='配置' disabled></input>" +
